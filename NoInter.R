...
lam=function(DD,BB1,BB2,BB3,alphas1,alphas2,bbetas,tx,txothers,year,depth){
	...
	intra=diag(alphas) #get intraspecific competition
	alphas[1:nrow(alphas),1:ncol(alphas)]=0 #set all alphas to zero
	diag(alphas)=intra #keep intraspecific competition
	alphaspre=alphas[,1:36]%*%tx 
	alphasothers=alphas[,37]*txothers
	alphasum=alphaspre+alphasothers
	txmas=log(tx+1)
	txmasother=log(txothers+1)
	bbetas[1:nrow(bbetas),1:ncol(bbetas)]=0 #set all betas to zero, note that there is no intraspecific facilitation in our model
